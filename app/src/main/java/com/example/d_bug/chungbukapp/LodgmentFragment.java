...
public class LodgmentFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ViewGroup rootView = (ViewGroup) inflater.inflate(R.layout.lodgment_fragment, container, false);
        ...
        thread = new Thread(new Runnable() {
            ...
            public void getXlsData(int row){
                ...
                imgURL = sheet.getCell(15, row).getContents();
                latitude = sheet.getCell(7, row).getContents();
                longitude = sheet.getCell(8, row).getContents();
                room = sheet.getCell(9, row).getContents();
                breakfast = sheet.getCell(10, row).getContents();
                creditCard = sheet.getCell(11, row).getContents();
                pickUp = sheet.getCell(12, row).getContents();
                parkingLot = sheet.getCell(13, row).getContents();
                facility = sheet.getCell(14, row).getContents();

                items.add(new LodgmentItem(title, tel, intro, cityName, address, time, siteURL, imgURL, latitude, longitude, room, breakfast, creditCard, pickUp, parkingLot, facility));
            }

            @Override
            public void run() {
                try {
                    InputStream inputStream = getActivity().getBaseContext().getResources().getAssets().open("xls_lodgment.xls");
                    ...
                    handler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            adapter = new MyAdapter(items, context, 4);
